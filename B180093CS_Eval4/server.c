...
int recvMessage(int i, char names[MAX_CLIENTS][100],int client_sockets[MAX_CLIENTS],int clientsNum){
  ...
  if(return_val<=0 || strcmp(client_message,"bye") == 0)
    return 0;
...
int getPassword(int i,int client_sockets[MAX_CLIENTS],char password[100]){
  char reply[100] = "wrong";
  char client_password[100];
  recv(client_sockets[i],client_password,100,0);
  printf("clientPassword: %s\n",client_password);
  if(strcmp(client_password,password) == 0){
    char right[100] ="right";
    send(client_sockets[i],right,strlen(right),0);
    return 1;
  }
  printf("---Error message sent---\n");
  printf("%s",reply);
  send(client_sockets[i],reply,strlen(reply),0);
  return 0;
}
  ...
  while(1){
    ...
    for(int i = 0;i<clientsNum;++i){
        if(FD_ISSET(client_sockets[i],&ready_sockets)){
            if(!names_given[i]){
                if(getPassword(i,client_sockets,password) == 0){
